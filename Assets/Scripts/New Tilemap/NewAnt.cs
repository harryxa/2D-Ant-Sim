...
public class NewAnt : MonoBehaviour
{
    ...
    private void Secrete()
    {
        ...
        Vector3Int tileGridPos = tilemap.WorldToCell(antPosition);

        //if (World.instance.GetTileAt((int)tileGridPos.x, (int)tileGridPos.y).type != Tile.Type.Grass)
        if (tilemap.GetTile(tileGridPos) != null)
        {
            return;
        }
        else
    ...
    ///NEEDS REFACTORING
    ///new grid system but needs to know which tiles to walk on etc.
    protected bool CheckTarget()
    {
        //Vector3 targetPos = World.instance.worldToTileGrid(targetPosition);
        Vector3Int targetPos = tilemap.WorldToCell(targetPosition);

        //Tile tile = World.instance.GetTileAt((int)targetPos.x, (int)targetPos.y);
        TileBase tile = tilemap.GetTile(targetPos);
